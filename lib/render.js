...
Render.prototype = {
    ...
    */
	clearMap: function() {
		...
		for (i=0 ; i < this.renderEvents.length ; i++) {
			Ticker.removeListener(this.renderEvents[i]);
		}
    ...
    */
	...
	map: function(data) {
		...
		this.containerMap = new Container();
							...
							for (j=0 ; j < map[0].length ; j++) {
								var id = map[i][j][l];
								 if (id != null) {
									...
									//console.log(autotiles_array.length, id);
									if (id - autotiles_array.length >= 0) {
										...
										bmpSeq.currentFrame = id-self.nb_autotiles * 48;
