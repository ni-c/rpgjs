...
RenderInterpreter.prototype = {
  ...
  cmdFadeOutAudio : function(types, params, self) {
    var bgm = self.render.soundVolume[types[0]], bgs = self.render.soundVolume[types[1]];
...
var p = {
  ...
  refresh : function(prop) {
    ...
    this.direction_fix = prop.direction_fix;
    // Direction does not change ; no animation
    this.no_animation = prop.no_animation;
    // no animation even if the direction changes
    this.stop_animation = prop.stop_animation;
    this.frequence = (prop.frequence || 0) * 5;
    this.speed = prop.speed === undefined ? 4 : prop.speed;
    this.sprite.z = prop.alwaysOnBottom ? 0 : false;
    // For z sort;
  ...
  displayEvent : function(graphic, only_change) {
    ...
    Cache.characters(graphic, function(chara) {
      ...
      var up = self.nbSequenceX * (self.nbSequenceY - 1) + self.graphic_pattern;
      // last line
      ...
      var frequency = Math.round(self.render.fps / self.speedAnimation);
      ...
      var anim = {
        ...
        walkRight : {
          frames : [right, right + nb],
          frequency : frequency
        },
        walkBottom : {
          frames : [bottom, bottom + nb],
          frequency : frequency
        },
        walkLeft : {
          frames : [left, left + nb],
      ...
      bmpSeq.waitFrame = frequency;

      self.sprite.regY = self.regY;
      self.sprite.regX = self.regX;

      if (!only_change)
        self.render.setPosition(self.id, self.x, self.y);
  ...
  animation : function(sequence, speed, onFinish, nbSequence, onChangeFrame) {// -C
    ...
    if (this.bitmap.currentAnimation != sequence) {
...
p.triggerEventBeside = function() {
  ...
  DataLink.emit("triggerEventBeside", {
    id : this.id
  });
}
...
Render.prototype = {
  ...
  tilePriority : function(tile_id) {
    if (!this.mapData.properties[tile_id]) {
      return 0;
    }
  ...
  map : function(data) {
    ...
    // Auotiles
    Cache.totalLoad["client"] += (properties.autotiles ? properties.autotiles.length : 0);
    ...
    function constructAutoTiles(seq, bmp, autotile, animated) {
      ...
      switch (seq) {
        ...
        case 4:
          ...
          break;
        case 5:
          var dir = [[autotile.top_left[0], autotile.top_right[1], autotile.right[2], autotile.left[3]], [autotile.top_left[0], autotile.top[1], autotile.bottom[2], autotile.bottom_left[3]], [autotile.left[0], autotile.right[1], autotile.bottom_right[2], autotile.bottom_left[3]], [autotile.top[0], autotile.top_right[1], autotile.bottom_right[2], autotile.bottom[3]]];
    ...
    function callback() {
      ...
      if (properties.autotiles) {
        ...
        for ( i = 0; i < properties.autotiles.length; i++) {
          img_autotiles = Cache.get(properties.autotiles[i], "autotiles");
          sprite = new createjs.SpriteSheet({
            images : [img_autotiles],
            frames : {
              width : self.tile_w / 2,
      ...
      if (!map_data.layer1) {
        var img_tileset = Cache.get(properties.tileset, "tilesets");
        var spriteSheet = new createjs.SpriteSheet({
          images : [img_tileset],
          frames : {
            width : self.tile_w,
      ...
      var img;
      for ( i = 0; i < 2; i++) {
        img = Cache.getMapGraphics(properties.filename, i);
        if (!img) {
          img = new Image();
          if (map_data.layer1) {
            img.src = map_data['layer' + (i + 1)];
            Cache.setMapGraphics(properties.filename, img);
          } else {
    ...
    function autotilesLoad(i) {
      Cache.autotiles(properties.autotiles[i], function() {
        progressLoad();
        if (properties.autotiles.length - 1 == i) {
          tilesetLoad();
        }
      });
    }
      ...
      if (properties[type]) {
        ...
        if (array) {
          if ( typeof properties[type] == 'string') {
            bg = array[1] != properties[type];
          } else {
            bg = array[1] != properties[type].mp3 + '.mp3' && array[1] != properties[type].ogg + '.ogg';
          }
          if (bg) {
            playBG(properties[type], type);
          }
        } else {
          playBG(properties[type], type);
        }
  ...
  addHtmlElement : function(html, x, y, event) {
    ...
    function displayElement(event) {
      ...
      if (element && event)
        event.htmlElements.push(element);
      self.htmlElements.push({
        element : element,
        event : event
      });
    }
   ...
   * @param {Object} properties The properties of the image :
   <ul>
   ...
   * @param {Object} properties New Property image. See "addPicture".
   */
